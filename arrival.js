...
export function createArrival({renderer,scene,camera,courtyard,controls,host,environment}){
 ...
 const initialEye=eye.position.clone(),initialTarget=new T.Vector3(0,.75,0);
 const suitcase=new T.Group();stage.add(suitcase);
 const bounds=new T.Box3().setFromObject(courtyard),size=bounds.getSize(new T.Vector3()),center=bounds.getCenter(new T.Vector3());
 const miniatureScale=Math.min(.103,3.50/size.x,2.94/size.z),miniaturePosition=new T.Vector3(-center.x*miniatureScale,.185-bounds.min.y*miniatureScale,-center.z*miniatureScale);
 const lidY=Math.max(.98,.185+size.y*miniatureScale+.12),wallHeight=lidY-.06;
 suitcase.name='回忆手提箱';suitcase.userData.fit={scale:miniatureScale,minY:.185,maxY:.185+size.y*miniatureScale,lidUnderside:lidY-.0805,width:size.x*miniatureScale,depth:size.z*miniatureScale};
 ...
 for(const x of [-1.89,1.89])block(.14,wallHeight,3.3,x,.035+wallHeight/2,0,leather);
 for(const z of [-1.59,1.59])block(3.78,wallHeight,.14,0,.035+wallHeight/2,z,leather);
 const lid=new T.Group();lid.name='手提箱盖';lid.position.set(0,lidY,-1.62);suitcase.add(lid);
 block(3.9,.19,3.3,0,.045,1.62,leather,lid);block(3.62,.035,3.02,0,-.063,1.62,lining,lid);
 for(const x of [-1.3,1.3]){block(.22,.025,3.27,x,.153,1.62,trim,lid);block(.25,wallHeight,.025,x,.035+wallHeight/2,1.667,trim);block(.30,.20,.055,x,lidY-.10,1.69,brass);block(.19,.12,.025,x,lidY-.10,1.727,trim);}
 ...
 for(const x of [-1.1,1.1])block(.35,.12,.13,x,lidY-.01,-1.63,brass);
 ...
 const model=new T.Group();model.name='箱内微缩院落';model.scale.setScalar(miniatureScale);model.position.copy(miniaturePosition);suitcase.add(model);model.add(courtyard);courtyard.visible=true;
 ...
 async function prepare(){
  ...
  scene.add(environment.root);lid.rotation.x=0;courtyard.visible=true;
 ...
 function start(){if(state!=='ready')return;open.blur();state='open';phase=lastNow;open.disabled=true;open.classList.add('depart');caption.textContent='院子还在，回忆就有了归处。';}
 ...
 return {update(now){if(finished)return false;if(preparing||awaitingEntry)return true;lastNow=now;if(started===null)started=now;const elapsed=(now-started)/1000;
  ...
  if(state==='open'){const t=clamp((now-phase)/(reduced?250:2000));lid.rotation.x=-ease(t)*Math.PI*.64;if(t===1){state='hold';phase=now;}}
  if(state==='hold'&&now-phase>=1200){state='travel';phase=now;stage.attach(model);[leather,trim,brass,lining,stitchMat].forEach(m=>{m.transparent=true;m.needsUpdate=true;});overlay.classList.add('travelling');stage.add(environment.root);environment.reveal(0);}
  initialEye.set(5.6,6.1,8.4).multiplyScalar(Math.max(1,1.05/(innerWidth/innerHeight)));
  ...
  if(state==='travel'){
   const growth=ease(clamp((travel-.30)/.70));model.scale.setScalar(T.MathUtils.lerp(miniatureScale,1,growth));model.position.copy(miniaturePosition).multiplyScalar(1-growth);const boxFade=1-T.MathUtils.smoothstep(travel,0,.28);suitcase.visible=travel<.30;[leather,trim,brass,lining,stitchMat].forEach(m=>m.opacity=boxFade);

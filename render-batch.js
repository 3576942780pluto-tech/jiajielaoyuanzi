...
export function batchStatic(root,movingRoots){
 root.updateMatrixWorld(true);const buckets=new Map(),moving=new Set(movingRoots),remove=[];
 root.traverse(o=>{
  ...
  const b=buckets.get(key),g=o.geometry,p=g.attributes.position,n=g.attributes.normal,uv=g.attributes.uv,off=b.p.length/3,normal=new T.Matrix3().getNormalMatrix(o.matrixWorld),v=new T.Vector3();
  for(let i=0;i<p.count;i++){
   v.fromBufferAttribute(p,i).applyMatrix4(o.matrixWorld);b.p.push(v.x,v.y,v.z);
